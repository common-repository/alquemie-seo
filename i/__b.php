...
class au_b{
    ...
    function __bo(){
        $this->__b('<div id="au_bc">','</div>');
        return;
    }
    function __bon($_i){
        $_o = get_option("au_seo_b");
        ...
        if(($_f == "page" && is_front_page()) || ($_f == "posts" && is_home())){
            ...
            if(is_tax() || is_tag() || is_category()){
                ...
                if(is_taxonomy_hierarchical($_t->taxonomy) && $_t->parent != 0){
                    ...
                    foreach($_ps as $_z) {
                        $_bct = _gtm($_z, $_t->taxonomy, 'au_bct_');
                        if(!$_bct)
                            $_bct = $_z->name;
                        $_c .= '<a href="'.get_term_link($_z, $_t->taxonomy ).'">'.$_bct.'</a> '.$_x.' ';
                    }
                }
                $_bct = _gtm($_t, $_t->taxonomy, 'au_bct_');
                if(!$_bct)
                    $_bct = $_t->name;
                if($paged)
...
if(!function_exists('au_breadcrumbs')){
    function au_breadcrumbs($_p = '', $_s = '', $_e = true) {
        $au_b = new au_b();
        $au_b->__b($_p, $_s, $_e);
    }	
}
