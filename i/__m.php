...
class au_m{
    ...
    function _ajs() {
        ...
        if (in_array($pagenow, array('post.php', 'page.php', 'post-new.php'))){
            wp_enqueue_script('au_seo_mb', AU_SEO_URL.'js/au_seo_mb.js', array('jquery'));
        }
    }
    ...
    function _acs() {
        global $pagenow;
        if (in_array($pagenow, array('post.php', 'page.php', 'post-new.php'))) {
            wp_enqueue_style('au_seo_mb', AU_SEO_URL.'css/au_seo_mb.css');
        }
    }
    ...
    function _emb($_mb) {
        ...
        if((!isset($_mbv) || empty($_mbv) ) && isset($_mb['_st_']) )  
            $_mbv = $_mb['_st_'];  
        echo '<tr><th><label for="au_seo'.$_mb['_nm_'].'">'.$_mb['_tt_'].':</label></th><td>';
        switch($_mb['_tp_']){ 
            case "x":
                echo '<input type="text" id="au_seo'.$_mb['_nm_'].'" name="'.$_mb['_nm_'].'" value="'.$_mbv.'" class="ti '.$_cl.'"/><br />';  
                break;
            case "a":
                $_r = 5;
                if(isset($_mb['_rw_']))
                    $_r = $_mb['_rw_'];
                echo '<textarea class="ta '.$_cl.'" rows="'.$_r.'" id="au_seo'.$_mb['_nm_'].'" name="'.$_mb['_nm_'].'">'.$_mbv.'</textarea>';
            break;
            case "s":
                echo '<select name="'.$_mb['_nm_'].'" id="au_seo'.$_mb['_nm_'].'" class="st '.$_cl.'">';
            ...
            case "m":
                ...
                echo '<select multiple="multiple" size="'.count($_mb['_opt_']).'" style="height: '.(count($_mb['_opt_'])*16).'px;" name="'.$_mb['_nm_'].'[]" id="au_seo'.$_mb['_nm_'].'" class="st '.$_cl.'">';
                foreach($_mb['_opt_'] as $_v=>$_o){
                    $_s = '';
                    if(in_array($_v,$_sa))
                        $_s = 'selected="selected"';
            ...
            case "r":
                ...
                foreach($_mb['_opt_'] as $_v=>$_o){
                    ...
                    if($_mbv == $_v)
                        $_s = 'checked="checked"';
                    echo '<input type="radio" '.$_s.' id="au_seo'.$_mb['_nm_'].$_v.'" name="'.$_mb['_nm_'].'" value="'.$_v.'"/> <label for="au_seo'.$_mb['_nm_'].'_'.$_v.'">'.$_o.'</label> ';
                }
            break;
        }
        if(isset($_mb['_de_']) )
        ...
        if(function_exists('add_meta_box') ) {  
            foreach(get_post_types() as $_t){
                ...
                if(isset($_os['_'.$_t.'-h-eb_']) && $_os['_'.$_t.'-h-eb_'])
                    continue;
                add_meta_box('au_seo_mb', 'Alquemie SEO', array(&$this, '_mbx'), $_t, 'normal', 'high' );  
            }
        }  
    }
