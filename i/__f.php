<?php
function _gsoa() {
    $_oa = array('au_seo','au_seo_t','au_seo_p','au_seo_i','au_seo_n','au_seo_r','au_seo_3','au_seo_e','au_seo_b','au_seo_h','au_seo_w','au_seo_f','au_seo_c');
    return $_oa;
}
function _gso() {
    $_o = array();
...
function _exps(){
    $_h = ";Backup file for Alquemie SEO. http://www.b2foundry.com\r\n"; 
    $_a = _gsoa();
    foreach($_a as $_og){
	$_h .= "\n".'['.$_og.']'."\n";
	$_os = get_option($_og);
    ...
    $_h  .= "\r\n\r\n[au_seo_m]\r\n";
    $_h .= "au_seo_m = \"".urlencode(json_encode(get_option('au_seo_m')))."\"";
    if(!$_hnd = fopen(au_SEO_UPD_DIR.'b2seo.ini', 'w'))
        die();
    if(!fwrite($_hnd, $_h)) 
        die();
    fclose($_hnd);
    require_once (ABSPATH . 'wp-admin/includes/class-pclzip.php');
    chdir(au_SEO_UPD_DIR);
    $_z = new PclZip('./b2seo.zip');
    if($_z->create('./b2seo.ini') == 0)
	return false;
    return au_SEO_UPD_URL.'b2seo.zip'; 
}

function _setup(){
    $_o = get_option('au_seo');
    $_u = false;
    if(!is_array($_o))
        $_o = array();
        $_u = true;
	...
	if(@is_writable($_os['_seo_updr_'])){
		$au_updr = $_os['_seo_updr_'];
		$au_upurl = $_os['_seo_upur_'];
	}else{
	    unset($_os['_seo_updr_']);
	    unset($_os['_seo_upur']);
	    update_option('au_seo', $_os);
	}
    }
    if(!isset($au_updr)){
	$_d = wp_upload_dir();
	...
	$au_updr = false;
	    ...
	    if($_dcr){
		$au_updr = $_d['basedir'].'/b2seo/';
		$_s = @stat(dirname($au_updr));
		$_dp = $_s['mode'] & 0007777;
		@chmod(dirname($au_updr),$_dp);
		$_os['au_updr_'] = $au_updr;
		$au_upurl = $_os['_seo_upur_'] = $_d['baseurl'].'/b2seo/';
		update_option('au_seo' , $_os);
	    }else{
	    ...
	    $au_updr = $_os['au_updr_'] = $_d['basedir'].'/b2seo/';
	    $au_upurl = $_os['_seo_upur_'] = $_d['baseurl'].'/b2seo/';
	    update_option('au_seo', $_os);
	}
	if($au_updr && @is_writable($au_updr)){
	    define('au_SEO_UPD_DIR', $au_updr);
	    define('au_SEO_UPD_URL', $au_upurl);
	} else {
	    define('au_SEO_UPD_DIR', false);
	    define('au_SEO_UPD_URL', false);
	    define('au_SEO_UPD_ERR', $_e);
	}
    }
    if($_u)
        update_option('au_seo', $_o);
    $_q = get_option('au_seo_e');
    $_x = $_q['_updfxx_'];
